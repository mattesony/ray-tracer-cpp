...
int floorAndClamp(int val){
    // TODO: Add logging here to make it obvious when clamping is happening
    if (0 > val){
        return 0;
    }
    ...
    for (int row = 0; row < this->height; row++){
        ...
        for (int column = 0; column < this->width; column++){
            ...
            for(int colorIndex=0; colorIndex<3; colorIndex++){
                std::string intStr = std::to_string(floorAndClamp(pixel(colorIndex)*MAX_COLOR_VALUE));
                if (70 < lineLen + int(0!=lineLen)+intStr.length()){
                    output << std::endl;
                    lineLen = 0;
                }
                lineLen += int(0!=lineLen)+intStr.length();
                if(0!=lineLen) output << " ";
                output << intStr;
            }
        }
